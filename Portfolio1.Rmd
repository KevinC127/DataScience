...
```{r}
Video_Game_Data <- read.csv("Video_Game_Sale_as_of_Jan_2017.csv")

```
(To get the data you need to download it from the github and place it same directory)
...
Critic_Score: Is the score usually give by know critic usually working in a reviewing company.(from in this case Metacritic)
Critic_Count: The number of critics used in coming up with the critic score.
User_score: The score given by individual in this case from Metacritic.
...
```
install ggplot2 for building graph plot
